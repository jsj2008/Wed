	...
	protected void doWakefulWork(Intent cmd) {
		...
		try {
			if (req.getPhase() == UpdateRequest.PHASE_DOWNLOAD) {
				...
				String updateVersionName = vcs.getVersionName();
				String currentVersionName = getPackageManager().getPackageInfo(
						getPackageName(), 0).versionName;

				if (updateVersionName.compareToIgnoreCase(currentVersionName) > 0) {
					...
					if (strategy == null
						...
						.compareToIgnoreCase(currentVersionName) < 0) {
					// TODO: file an error
				}
			}
		} catch (Exception e) {
