...
public final class Plist {
	...
	/**
	 * Convert a nested {@code map<String, Object>} as a plist xml string using
	 * the default mapping.
	 * 
	 * @param data
	...
	/**
	 * Store a nested {@code map<String, Object>} as a plist using the default
	 * mapping.
	 * 
	 * @param data
	 *            the nested data to store as a plist.
	 ...
	 * @throws IOException
	 *             if there was an IO error saving the file.
	 */
	 ...
	 * 
	 * @param object
	 *            the object to store as a plist.
	 * @param filename
	 *            the destination file to store the data to.
	 * @throws IOException
	 *             if there was an IO error saving the file.
	...
	/**
	 * Create a nested {@code map<String, Object>} from a plist xml string using
	 * the default mapping.
	 * 
	 * @param xml
	...
	/**
	 * Create a nested {@code map<String, Object>} from an XmlElement using the
	 * default mapping.
	 * 
	 * @param element
	 *            the plist xml data as a string.
	 ...
	 *             if the plist could not be properly parsed.
	 */
	public static Map<String, Object> fromXmlElement(XmlElement element)
			throws XmlParseException {
	 ...
	 *             if the plist could not be properly parsed.
	 */
	public static Object objectFromXmlElement(XmlElement element)
			throws XmlParseException {
	 ...
	 */
	public static Map<String, Object> load(File file) throws XmlParseException,
			IOException {
	 ...
	 *             if there was an issue reading the plist file.
	 */
	public static Object loadObject(File file) throws XmlParseException,
			IOException {
	 ...
	 *             if there was an issue reading the plist file.
	 */
	public static Object loadObject(String filename) throws XmlParseException,
			IOException {
	 ...
	 * 
	 * @param o
	 *            the object to convert, must be Integer, Double, String, Date,
	 *            Boolean, byte[], Map or List.
	 * @return an <tt>XmlElement</tt> containing the serialized version of the
	 *         object.
	 */
	...
	XmlElement objectToXml(Object o) {
		...
		if (o instanceof Map) {
			...
			throw new RuntimeException("Cannot use " + o.getClass()
					+ " in plist.");
	}
	 ...
	 * 
	 * @param map
	 *            the map to convert, assumed to have string keys.
	...
	Object parseObject(XmlElement element) throws XmlParseException {
		if (!"plist".equalsIgnoreCase(element.getName()))
			throw new XmlParseException("Expected plist top element, was: "
					+ element.getName());
	...
	private Object parseElement(XmlElement element) throws XmlParseException {
		try {
			...
			throw new XmlParseException("Failed to parse: " + element.toXml(),
					e);
		}
	}
	...
	static String base64encode(byte[] bytes) {
		...
		for (int i = 0; i < bytes.length; i += 3) {
			...
			builder.append(i < bytes.length - 1 ? BASE64_CHARS[((b1 & 0x0F) << 2)
					| ((b2 & 0xC0) >> 6)]
					: "=");
	...
	static byte[] base64decode(String base64) {
		...
		if (base64.length() % 4 != 0)
			throw new IllegalArgumentException("Illegal base64 string, length "
					+ base64.length());
