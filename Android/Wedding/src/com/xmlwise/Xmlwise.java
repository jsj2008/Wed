...
public class Xmlwise {
	 ...
	 *             if there is a problem parsing the xml in the file.
	 */
	 ...
	 *            the file to read from.
	 * @param validate
	 *            if we should validate the document or not.
	 ...
	 *             if there is a problem parsing the xml in the file.
	 */
	public static Document loadDocument(File file, boolean validate,
			boolean loadExternalDTD) throws IOException, XmlParseException {
		try {
	 ...
	 *             if we fail to setup the builder.
	 */
	private static DocumentBuilder getBuilderFactory(boolean validate,
			boolean loadExternalDTD) throws ParserConfigurationException {
	...
	/**
	 * Creates a DOM Document from the specified XML string, ignoring
	 * DTD-validation.
	 * 
	 * @param xml
	 ...
	 * @throws XmlParseException
	 *             if we fail to parse the XML.
	 */
	public static Document createDocument(String xml, boolean validate,
			boolean loadExternalDTD) throws XmlParseException {
		try {
			return getBuilderFactory(validate, loadExternalDTD).parse(
	 ...
	 * @throws IOException
	 *             if there were any problems reading from the file.
	 */
	public static XmlElement loadXml(File file) throws XmlParseException,
			IOException {
	 ...
	 *             if there were any problems reading from the file.
	 */
	public static XmlElement loadXml(String filename) throws XmlParseException,
			IOException {
		return loadXml(new File(filename));
	}

	/**
	 * Creates a document from a string and transforms it into an XmlElement
	 * tree.
	 * 
	 * @param xml
	 *            the xml as a string.
